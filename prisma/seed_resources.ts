...
async function main() {
    ...
    // 1. Chair-Based Exercise Video Series
    const chairExercises = await prisma.resourceCategory.create({
        data: {
...
Move gently. Stay consistent. And listen to your body.`,
    ...
    for (const item of chairItems) {
        await prisma.resourceItem.create({
            data: {
                categoryId: chairExercises.id,
                ...item,
            },
        });
    }

    // 2. Walking Programme Video Series
    const walkingProgram = await prisma.resourceCategory.create({
        data: {
...
Always prioritize your safety and comfort.`,
    ...
    for (const item of walkingItems) {
        await prisma.resourceItem.create({
            data: {
                categoryId: walkingProgram.id,
                ...item,
            },
        });
    }

    // 3. Educational Animation Series
    const educationalSeries = await prisma.resourceCategory.create({
        data: {
...
Always follow your doctor's instructions for your medications.`,
    ...
    for (const item of eduItems) {
        await prisma.resourceItem.create({
            data: {
                categoryId: educationalSeries.id,
                ...item,
            },
        });
    }
